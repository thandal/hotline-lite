  ...
  if (context.BLOCKLIST.split(',').includes(event.From)) {
    ...
    for (let n = 0; n < 2; n++) {
      for (let i = 0; i < languages.length; i++) {
        ...
        gather.say({ language: langToLangLocale[languages[0]] }, messagesMap[languages[i]].caller.welcome.hello);
        sayLangMap(
          gather,
          languages[i],
    ...
    if (languages.length == 1) {
      ...
      twiml.say({ language: langToLangLocale[languages[0]] }, messagesMap[key].caller.welcome.hello);
    } else {
